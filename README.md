...
**AI for Biology / 蛋白质 / 多肽 / 大语言模型 / 生成模型** 相关的新论文与新闻，由 **DeepSeek** 精筛和点评后，推送到你的 **微信**。
...
- 微信推送：支持 Server酱 / 企业微信群机器人 / PushPlus
...
2. **微信推送渠道**（三选一）
   ...
   | 方式 | 适合 | 需要什么 |
   |---|---|---|
   | Server酱（推荐个人微信） | 个人微信收消息 | 到 https://sct.ftqq.com 用 GitHub 登录后获得 SendKey |
   | 企业微信群机器人 | 微信群 | 创建企业微信 -> 添加群机器人 -> 得到 Webhook URL |
   | PushPlus | 个人微信 | http://www.pushplus.plus 获取 token |

   > 个人微信本身没有开放官方自动推送 API，所以常用上述第三方渠道中转。
...
| `WECHAT_PUSH_TYPE` | `serverchan` / `wecom` / `pushplus` | 必填 |
...
打开仓库 `Actions` 页 -> 选择 **Daily AI Bio Digest** -> 点 **Run workflow**，等跑完看微信是否收到。
...
### 5. 我的 DeepSeek Key 已经写在聊天里了，安全吗？
